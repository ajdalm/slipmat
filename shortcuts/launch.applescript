...
on run argv
	...
	if theURL is "" then
		if mode is "spotify" then
			display alert "SLIPMAT — SPOTIFY" message "Clipboard is empty. Copy a Spotify playlist, album or track link (in Spotify: Share), then run again." as warning
	...
	if mode is "best" then
		...
		if theURL does not contain "spotify" then
			display alert "SLIPMAT — SPOTIFY" message "That's not a Spotify link. Copy a Spotify playlist, album or track link (in Spotify: Share), then run again." as warning
			return
		end if
